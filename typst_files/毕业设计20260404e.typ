...
= 使用示例
本章使用的 api 配置是 DeepSeek-CHat V3.2 的官方渠道。

#figure(
  image("flowcabal_llmconfig.png"),
  caption: [api 配置]
)
...
flowcabal 的内置 agent 定制了 tool set，因此添加手稿要通过软件命令。

#figure(
  image("flowcabal_memory_agent1.png"),
  caption: [添加手稿]
)

#figure(
  image("flowcabal_memory_agent2.png"),
  caption: [与 agent 聊天]
)

为了下一节测试方便，现在让 agent 编写两个记忆文件。

#figure(
  image("flowcabal_memory_agent3.png"),
  caption: [agent 管理记忆文件]
)

== workflow

首先创建一个含有 agent 注入项的节点并运行。
...
当前由于软件功能支持有限，故用 debug 方式展示 agent 注入的提示词：

```bash
Running...
[DEBUG resolvePrompt] wu5iz3a8l9nzr0q8bvfp29q3fcd3wx3o39k4 system:
[DEBUG resolvePrompt] wu5iz3a8l9nzr0q8bvfp29q3fcd3wx3o39k4 user:
下面是这个小说的所有角色，请你按照角色在故事中的位置将它们分类
根据记忆文件，这个小说的主要角色有：
## 主要角色
### 1. **欢愉之主**
- **身份**：能够攀上存在之树观察宇宙的神祇
- **能力**：攀爬存在之树、观察宇宙本质、笑声具有改变宇宙的力量
- **性格**：敏锐的观察者，情感丰富，具有幽默感
- **关键作用**：看到婴儿啼哭后纵声大笑，笑声撕裂了冰冷死寂的宇宙，至今仍在诸界回荡
### 2. **婴儿**
- **身份**：神秘的新生婴儿
- **特征**：刚刚落地啼哭，哭声"犹如受尽委屈"
- **象征意义**：生命的脆弱、情感的真实、人性的代表
- **关键作用**：婴儿的啼哭与冰冷机械的宇宙形成鲜明对比，触发了欢愉之主的大笑，成为改变宇宙命运的关键触发点
这两个角色构成了故事的核心冲突和转变：
- **欢愉之主**代表观察者和改变者
- **婴儿**代表触发改变的人性元素
他们的互动（婴儿啼哭 → 欢愉之主大笑 → 宇宙改变）是整个故事的核心事件。
Executed 1 nodes: wu5iz3a8l9nzr0q8bvfp29q3fcd3wx3o39k4
...
#figure(
  image("workflow2.png"),
  caption: [node：主要角色命运扩展]
)

#figure(
  image("workflow3.png"),
  caption: [node：次要角色命运扩展]
)
